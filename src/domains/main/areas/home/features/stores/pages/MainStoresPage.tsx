...
const categoryOptions: CategoryOption[] = [
  { value: '1', label: '패션잡화', subCategories: [
  ...
  { value: '2', label: '인테리어 소품', subCategories: [
      { value: '6', label: '패브릭' },
      { value: '7', label: '꽃/식물' },
      { value: '8', label: '조명' },
      { value: '9', label: '인테리어 소품 기타' },
    ] },
  { value: '3', label: '악세서리', subCategories: [
      { value: '10', label: '반지' },
      { value: '11', label: '팔찌' },
      { value: '12', label: '귀걸이' },
      { value: '13', label: '악세서리 기타' },
    ] },
  { value: '4', label: '케이스/문구', subCategories: [
      { value: '14', label: '폰케이스' },
...
const MainStoresPage: React.FC = () => {
  ...
  const fetchAllStores = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        const stores = await Promise.all(res.data.map(async s => {
          const catRes = await legacyGet<{ data: { ucategory?: string; dcategory?: string } }>(`/main/${s.storeId}/categories`);
          const c = catRes.data ?? {};
          return {
            id: String(s.storeId),
            name: s.storeName,
  ...
  const filteredAll = useMemo(()=>{
    return allStores.filter(s=>{
      ...
      const subOk = subCatList.length === 0 
        || subCatList.some(value => VALUE_TO_LABEL[value] === s.downCategory || VALUE_TO_LABEL[value] === s.upCategory);
  ...
  const handleSubCatToggle = (value:string)=>{
    setSubCatList(prev=>prev.includes(value)? prev.filter(v=>v!==value) : [...prev,value]);
  };
        ...
        <div className="w-full max-w-[1440px] mx-auto py-2">
          ...
          {showCategoryFilter && (
            <div className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
              {categoryOptions.filter(c=>c.value!=='5').map(c=>(
