...
const ClassStatsCard: FC<Props> = ({
  ...
  const prevModeRef = useRef<ClassStatsMode>(mode);
    const prevWeekRef = useRef<string>(weekValue);      
  const shouldAnimate =
    firstMountRef.current ||
    prevModeRef.current !== mode ||
    prevWeekRef.current !== weekValue;      
  ...
  const showWeek =
    showWeekInputWhen === "always" ||
    (showWeekInputWhen === "weekday" && mode === "weekday");
