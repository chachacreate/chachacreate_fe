...
// import heroUrl from '@src/domains/main/areas/home/features/main-landing/assets/classbg.webp'; // 더 이상 사용하지 않음
...
export default function MainClassesPage() {
  ...
  return (
    <>
      ...
      <section
        ref={heroSectionRef}
        className="relative w-full overflow-hidden bg-white min-h-[calc(100vh-64px-34px)] lg:min-h-[calc(100vh-64px-34px)] isolation-auto"
        aria-label="클래스 상단 뷰(히어로/캘린더)"
      >
        <div
          className={[
            'flex w-[200%] h-full transition-transform duration-700 ease-out will-change-transform',
    ...
    <div className="h-full w-full flex items-center justify-center relative z-10">
      <div className="relative w-full flex items-center min-h-[calc(100vh-64px-64px)] lg:min-h-[calc(100vh-64px-34px)]">
        
        <div className="w-full max-w-6xl mx-auto text-center px-6 lg:px-8">
          
          {/* 메인 타이틀 - 훨씬 더 세련되게 */}
          <div className="relative mb-12">
            <div className="relative">
              {/* 글로우 효과 배경 */}
              <div className="absolute -inset-2 bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 blur-3xl rounded-3xl animate-pulse"></div>
              
              <h1 className="relative text-5xl md:text-7xl lg:text-9xl font-black leading-none tracking-tight">
                {/* 첫 줄 */}
            <div className="mb-4 transition-transform duration-700">
              <span
                className="
                  ...
                  drop-shadow-[0_10px_2px_rgba(0,0,0,0.55)]
              ...
              <span
                className="
                  mx-4 text-white/95 font-light
                  drop-shadow-[0_10px_10px_rgba(0,0,0,0.85)]
            ...
            <div className="transition-transform duration-700">
              <span
                className="
                  text-white/90 font-light text-6xl md:text-8xl lg:text-9xl
                  drop-shadow-[0_10px_20px_rgba(0,0,0,0.85)]
              ...
              <span
                className="
                  ...
                  bg-clip-text text-transparent
                  drop-shadow-[0_10px_2px_rgba(0,0,0,0.55)]
                  motion-safe:animate-pluse
                  [animation-delay:600ms] [animation-duration:3s]
                "
              >
                Professionals
              </span>
          ...
          <div
            ...
            className="w-1/2 h-full flex justify-center bg-gradient-to-br from-gray-50 to-white relative z-10"
          >
            <div className="w-full h-full flex items-center justify-center mx-auto px-4 py-12 md:px-6 lg:px-8">
              <div className="w-full max-w-5xl animate-fade-in">
                <div className="text-center mb-8">
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">이번 달 일정</h2>
                  <p className="text-gray-600">다가오는 클래스들을 확인해보세요</p>
                </div>
                <CalendarPreview items={items.slice(0, 12)} />
                <p className="mt-6 text-sm text-gray-500 text-center bg-white/50 backdrop-blur-sm rounded-lg p-3">
      ...
      <main className="relative">
        ...
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 md:px-8 lg:px-16 xl:px-[240px]">
          <div className="max-w-[1440px] mx-auto">
            ...
            <div id="search-section" className="py-8 lg:py-14" ref={searchSectionRef} />

            {/* 검색바 + 초기화 */}
            <section className="mb-6 lg:mb-8 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
              <div className="mb-4 text-center">
                <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">클래스 찾기</h2>
                <p className="text-gray-600">원하는 클래스를 검색하고 정렬해보세요</p>
              </div>
              
              <div
                ref={searchbarWrapRef}
                onSubmitCapture={onSearchbarSubmitCapture}
                className="mx-auto w-full max-w-2xl flex items-center gap-3"
              >
                ...
                <button
                  type="button"
                  onClick={onResetAll}
                  className="h-11 px-4 rounded-xl border border-gray-300 bg-white text-sm font-medium hover:bg-gray-50 hover:shadow-md active:scale-95 transition-all duration-200"
                  aria-label="검색 초기화"
                >
                  초기화
                </button>
            ...
            {/* 정렬바 */}
            <section className="mb-8 lg:mb-10 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
...
function SortBar({
  ...
  return (
    <div className="w-full overflow-x-auto scrollbar-hide">
      <div className="inline-flex items-center gap-1 border border-gray-200 rounded-2xl p-1.5 bg-white shadow-sm hover:shadow-md transition-shadow duration-300 min-w-max mx-auto">
        {items.map((it, index) => {
          ...
          return (
            <button
              ...
              className={[
                'h-10 px-4 rounded-xl text-sm font-medium transition-all duration-300 whitespace-nowrap',
                active 
...
function CalendarPreview({ items }: { items: ClassItem[] }) {
  return (
    <div className="rounded-3xl border border-gray-200/60 p-8 bg-white/90 backdrop-blur-sm shadow-2xl hover:shadow-3xl transition-all duration-500">
      <div className="max-h-[480px] overflow-auto scrollbar-hide">
        {items.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-lg mb-2">예정된 클래스가 없습니다</p>
            <p className="text-gray-400 text-sm">곧 새로운 클래스가 개설될 예정입니다</p>
          </div>
        ) : (
          <ul className="space-y-4">
            {items.map((cls, index) => (
              <li 
                key={cls.id} 
                className="group bg-white/50 rounded-2xl p-5 hover:bg-white hover:shadow-lg transition-all duration-300 animate-fade-in-up border border-gray-100/50"
                style={{ animationDelay: `${index * 0.08}s` }}
              >
                <div className="grid grid-cols-1 lg:grid-cols-[140px_1fr_auto] items-center gap-4">
                  {/* 날짜 */}
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-[#2d4730]/10 rounded-xl flex items-center justify-center">
                      <svg className="w-6 h-6 text-[#2d4730]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                    </div>
                    <div className="text-left">
                      <div className="text-sm font-semibold text-[#2d4730]">
                        {cls.startDate}
                      </div>
                  ...
                  <div className="space-y-2">
                    <h3 className="font-bold text-lg text-gray-900 group-hover:text-[#2d4730] transition-colors line-clamp-1">
                      {cls.title}
                    </h3>
                  ...
                  <Link
                    ...
                    state={{ item: cls }}
                    className="inline-flex items-center justify-center h-11 px-6 rounded-xl border-2 border-[#2d4730]/20 bg-white hover:border-[#2d4730] hover:bg-[#2d4730] hover:text-white transition-all duration-300 text-sm font-semibold active:scale-95 group-hover:shadow-lg"
                  >
                    예약하기
...
function ListView({ items, isLoading }: { items: ClassItem[]; isLoading: boolean }) {
  if (isLoading) {
    return (
      <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
  ...
  if (items.length === 0) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-lg text-gray-500 mb-2">등록된 클래스가 없습니다</p>
    ...
    <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
      {items.map((cls, index) => (
        <Link
          ...
          to={`/main/classes/${cls.id}`}
          state={{ item: cls }}
          className="block group focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#2d4730] rounded-2xl animate-fade-in-up"
          style={{ animationDelay: `${index * 0.05}s` }}
        >
          <article className="border border-gray-200 rounded-2xl overflow-hidden bg-white transition-all duration-500 group-hover:shadow-2xl group-hover:-translate-y-2 group-hover:border-[#2d4730]/30">
            <div className="aspect-[4/3] bg-gray-100 overflow-hidden relative">
              ...
              {/* 남은 자리 배지 */}
              {cls.remainSeat <= 3 && cls.remainSeat > 0 && (
                <div className="absolute top-3 right-3 bg-red-500 text-white text-xs px-2 py-1 rounded-full font-bold animate-bounce-in">
                  마감임박 {cls.remainSeat}석
                </div>
              )}
            </div>
            ...
            <div className="p-4 md:p-5">
              <h3 className="font-bold text-lg line-clamp-1 group-hover:text-[#2d4730] transition-colors duration-300 mb-2">
                {cls.title}
              </h3>
              ...
              <div className="flex items-center justify-between mb-4">
                <p className="text-xl font-bold text-gray-900 group-hover:text-[#2d4730] transition-colors">
                  {cls.price.toLocaleString()}원
                </p>
                {cls.remainSeat > 0 && (
              ...
              <div className="mt-4">
                <span className="inline-flex w-full h-11 items-center justify-center rounded-xl border border-gray-300 bg-white group-hover:bg-[#2d4730] group-hover:text-white group-hover:border-[#2d4730] transition-all duration-300 text-sm font-semibold active:scale-95">
                  자세히 보기
                  <svg className="w-4 h-4 ml-2 transform group-hover:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
...
function Pagination({
  ...
  return (
    <nav className="mt-12 mb-16 flex items-center justify-center" aria-label="클래스 페이지네이션">
      <div className="flex items-center gap-2 bg-white rounded-2xl border border-gray-200 p-2 shadow-lg hover:shadow-xl transition-shadow duration-300">
        {/* 이전 버튼 */}
        <button
          onClick={() => go(page - 1)}
          disabled={!canPrev}
          className={[
            'h-10 px-4 rounded-xl border transition-all duration-200 font-medium text-sm',
            canPrev 
